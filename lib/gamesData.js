...
export async function fetchSeasonGameLog(league, season, variant, { type } = {}) {
  ...
  return { games: pairGameRows(allRows), error: null };
}
...
function resolveWinsNeeded(leagueConfig, season) {
  ...
  const configuredMax = Object.keys(roundLabels)
    .map(Number)
    .filter((n) => !Number.isNaN(n) && n >= 1)
    .reduce((max, n) => Math.max(max, n), null);
...
// itself (the highest round number actually played that season), not
// from leagueConfig's static roundLabels max. WNBA's playoff STRUCTURE
// has changed over time, not just series length - 1997 only had 2 real
// rounds (no "First Round" existed yet), so that year's actual
// championship game is stored under round=2, the same number today's
// 3-round Semifinals is. A team that legitimately won the 1997 title
// would never trip a fixed "highestRound === 3" check, no matter how
// many games they won, because their season's bracket never went past
// round 2. Comparing each team's highestRound against ITS OWN season's
// observed max (capped at the league's configured max as a sanity
// ceiling) fixes this without needing to hardcode which years had which
// round structure - it just reads it off the actual games played.
//
...
export function tallyPlayoffResults(poGames, leagueConfig, keyFn = (row) => row.team_id) {
  ...
  // Real max round PLAYED per season, capped at configuredMax as a sanity
  // ceiling (guards against a stray bad round value inflating this past
  // what the league's format actually supports).
  const seasonMaxRound = {};
  for (const row of poGames) {
    const season = row.season;
    const rndNum = Number(row.round);
  ...
  for (const t of Object.values(byKey)) {
    const seasonFinalsRound = t.season != null ? seasonMaxRound[t.season] : configuredMax;
    if (seasonFinalsRound != null && t.highestRound === seasonFinalsRound) {
      const needed = resolveWinsNeeded(leagueConfig, t.season);
      const w = t.rounds[String(seasonFinalsRound)]?.w ?? 0;
...
async function fetchAllRowsForTeam(selectCols, filters) {
  ...
  while (true) {
    let q = supabase.from("games").select(selectCols);
    for (const [col, val] of Object.entries(filters)) {
      if (Array.isArray(val)) q = q[val[0]](col, val[1]);
      else q = q.eq(col, val);
    }
    const { data, error } = await q.order("date", { ascending: true }).range(from, from + PAGE_SIZE - 1);
    if (error) return { rows: [], error };
    allRows = allRows.concat(data ?? []);
    if (!data || data.length < PAGE_SIZE) break;
    from += PAGE_SIZE;
