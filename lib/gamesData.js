...
// The date GamesPanel should default to showing. Prefers the most recent
// COMPLETED game date, since that's what "latest" means once a season is
// underway. But a season can be fully scheduled with predictions loaded
// before it's played a single game (see getCurrentSeason above) - in that
// case there's no "latest completed date" to fall back to, and returning
// null here means GamesPanel's date-selection effect never fires, its
// game-fetching effect never runs, and the panel just sits in "loading"
// forever with nothing shown. So when there's no completed game yet, use
// the EARLIEST scheduled date instead (i.e. the season's Week 1) so the
// panel has something concrete to select and load - which naturally hits
// the scheduled-games branch (getScheduledGamesForDate) and shows the
// upcoming slate with spreads/win probabilities.
...
// table before any real data for it actually exists. Trusting that row
// as-is could pick a truly empty season - every query scoped to it then
// legitimately returns nothing.
//
// "Ready" means EITHER completed games OR a loaded schedule/predictions
// exist for the season - a preseason week (0-0 records, Week 1 spreads
// and win probabilities already computed from preseason_ratings) is a
// legitimate, intentional display state, not the empty-page case this
// guards against. Only a season with neither is skipped in favor of the
