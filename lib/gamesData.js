...
// season + round, consulting playoffFormat.winsNeededByEra when the league
// defines it (WNBA's Finals format has changed several times across
// history: 1997 single game, 1998-2004 best-of-3, 2005-2024 best-of-5,
// 2025+ best-of-7 — sourced Aug 2026, see wnba/config.js). Falls back to
// the league's static winsNeeded (or 4, i.e. best-of-7) for leagues/rounds
// that don't have era data, which covers the NBA (every round has always
// been best-of-7, so there's nothing era-dependent to model).
function resolveWinsNeeded(leagueConfig, season, round) {
  ...
  if (eras?.length && season != null) {
    ...
    if (era?.winsNeeded?.[round] != null) return era.winsNeeded[round];
  }
  return leagueConfig.playoffFormat?.winsNeeded?.[round] ?? 4;
}
...
// and fetchAllTimePlayoffGames select it) so the era lookup always has
// what it needs regardless of which page is calling this.
export function tallyPlayoffResults(poGames, leagueConfig, keyFn = (row) => row.team_id) {
  ...
  for (const t of Object.values(byKey)) {
    if (t.highestRound === highestRound && highestRound !== null) {
      const needed = resolveWinsNeeded(leagueConfig, t.season, highestRound);
