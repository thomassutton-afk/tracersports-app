...
// Cached per league — the flat public/logos/historical/ folder can only
// safely serve one league until WNBA files move into their own subfolder
// (see app/api/logo-index/route.js), but caching per league now means no
// code change is needed here once that split happens.
...
// The actual historical logo files (public/logos/historical/*.png) use the
// OLD site's shorter team_id convention in a few cases, not the new site's
// (see nba/config.js's header comment: NYK/GSW/SAS/NOP/CHH here vs.
// NY/GS/SA/NO/CHA on the old site) — same underlying franchises, different
// filename prefix. Checked against the real files in the repo rather than
// assumed. Applied only at logo-lookup time; display name/abbreviation
// elsewhere in the app is unaffected.
const LOGO_CODE_ALIASES = {
  GSW: "GS",   // Warriors — never relocated, no team_history row, but files use "GS"
  NOP: "NO",   // Pelicans' current (2014-present) era
  NOH: "NO",   // New Orleans Hornets era (2003-2013) — filed under "NO" too, not "NOH"
  NOK: "NO",   // "New Orleans/OKC Hornets" 2006-2007 — also filed under "NO"
  NYK: "NY",
  SAS: "SA",
  NJN: "NJ",   // old New Jersey Nets — files use "NJ"
  CHH: "CHA",  // current (2015-present) Charlotte Hornets — files use "CHA"
};
...
export function resolveHistoricalLogoPath(teamId, season, historyByTeam, logoIndex) {
  const eras = historyByTeam[teamId];
  ...
  for (const { code: rawCode, maxYear } of segments) {
    const code = LOGO_CODE_ALIASES[rawCode] ?? rawCode;
    const cap = Math.min(season, maxYear);
    const years = logoIndex?.[code] ?? [];
    for (const year of years) {
      if (year <= cap && year > bestYear) {
        bestYear = year;
        bestFile = `/logos/historical/${code}_${year}.png`;
      }
    }
  }
