...
export const wnbaConfig = {
  ...
  playoffFormat: {
    ...
    // WNBA Finals length has changed multiple times across league history
    // (sourced Aug 2026 via ESPN/WNBA.com/Wikipedia):
    //   1997        — single game                  (1 win needed)
    //   1998–2004   — best-of-3                     (2 wins needed)
    //   2005–2024   — best-of-5                     (3 wins needed)
    //   2025+       — best-of-7                     (4 wins needed)
    // tallyPlayoffResults (lib/gamesData.js) uses this to correctly flag
    // champions across every era instead of applying the current-era
    // winsNeeded retroactively — which was silently marking every
    // pre-2025 champion as "not champion" on the All-Time page (only
    // the 2025 Aces, who actually won a 4-game Finals, passed the old
    // static check). Only Finals (round 3) is modeled since winsNeeded is
    // only consulted for the highest round when setting the champion flag
    // — Round 1/Semis win totals display as-is regardless of era.
    winsNeededByEra: [
      { fromSeason: 1997, toSeason: 1997, winsNeeded: { 3: 1 } },
      { fromSeason: 1998, toSeason: 2004, winsNeeded: { 3: 2 } },
      { fromSeason: 2005, toSeason: 2024, winsNeeded: { 3: 3 } },
      { fromSeason: 2025, toSeason: null, winsNeeded: { 1: 2, 2: 3, 3: 4 } },
    ],
  },
