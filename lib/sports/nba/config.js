...
export const nbaConfig = {
  ...
  // pairGameRows(). NBA scores are always comfortably above this.
  minValidGameScore: 50,
  ...
  engine: {
    ...
    // Ordered elimination-bracket progression (excludes RS/Play-In/
    // In-Season Tournament — those aren't part of the bracket depth).
    // Used wherever round "depth" needs comparing (champion detection,
    // All-Time depth filters, etc.) instead of parsing the round value
    // as a number directly — see lib/gamesData.js's roundRank(). For
    // NBA this array happens to just be the numbers as strings, so
    // roundRank produces identical results to the old Number(round)
    // approach; the array exists so leagues with non-numeric round
    // codes (NFL: 'WC'/'DV'/'CC'/'SB') work the same way without a
    // separate code path.
    playoffRoundOrder: ['1', '2', '3', '4'],
  },
};
