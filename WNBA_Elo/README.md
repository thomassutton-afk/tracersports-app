...
## Checking specific seasons only

```
python3 check_season.py --season 2001
python3 check_season.py --season 2001 --season 2002 --season 2003
```

Read-only. For each season given, prints its game count, sanity checks
(no NaN ratings, no team missing from standings), accuracy/Brier/log
loss for just that season's games, and its standings. Says clearly if
a season isn't in the database yet rather than erroring.

For an exact column-by-column check against an Excel backup for one
season, use `verify_against_workbook.py` instead (below).
...
## Handling a season that's in progress (schedule, predictions, projections)

**The core safeguard:** unplayed games live in their own `schedule`
table, never in `games`. The rating engine only ever reads from
`games` - it has no code path that touches `schedule` at all, so an
unplayed game structurally cannot become a fake result (this is
specifically to avoid the "blank score = 0-0 tie" bug some spreadsheet
Elo models fall into).

**Workflow:** pull the full season's schedule the day it's released
and load it, then just re-run the same command daily as scores come
in:

```
python3 add_season.py Results/WNBA_2025_Results.xlsx
```

A row with a real score goes into `games` and affects ratings, as
always. A row with everything else but no score yet goes into
`schedule` instead - not an error, not a warning, just routed there
automatically. Once a game gets its real score in a later file, it's
inserted into `games` and its `schedule` placeholder is automatically
removed. You don't do anything differently - it's the same command
every time regardless of whether the file has 3 finished games or 300
scheduled ones.

**Predicting upcoming games:**

```
python3 predict.py --season 2025
```

Uses each team's current rating (as of the most recently played real
game) to give a win probability for every unplayed game in the
schedule. Read-only, doesn't touch ratings or the schedule table.
Writes `reports/upcoming_predictions.csv`.
...
Monte Carlo projection: simulates the remaining schedule thousands of
times, drawing each game's winner from the model's own win probability
and a plausible margin sampled from real historical games, updating
ratings game-by-game within each simulated trial (so a simulated
winning streak actually raises a team's rating for its own later
simulated games, same as reality). Reports projected final wins,
10th-90th percentile range, and probability of finishing 1st, per
team. Writes `reports/season_projection_<season>.txt` and `.csv`. This
is a projection based on current ratings, not a guarantee - treat it
as a plausible range of outcomes.
