...
def test_preview_matchup_does_not_mutate_state():
    """Predicting an upcoming game must never change any team's actual
    rating/state - that's the whole point of it being separate from
    process_game."""
    eng = EloEngine(DEFAULT_PARAMS)
    eng.process_game(dict(
        date=date(1997, 6, 21), season=1997, type="R", round=None,
        home_team="A", away_team="B", home_pts=80, away_pts=70, ot=0,
    ))
    before = dict(eng.current_ratings())

    eng.preview_matchup(home_team="A", away_team="B", game_date=date(1997, 6, 23), season=1997)
    eng.preview_matchup(home_team="A", away_team="C", game_date=date(1998, 6, 11), season=1998)

    after = dict(eng.current_ratings())
    assert before == after, f"preview_matchup mutated state: {before} -> {after}"
    assert "C" not in eng.teams, "preview_matchup should not register a team it merely previewed"
