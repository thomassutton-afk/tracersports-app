"""
ContinElo vs. 538/Paine Elo — Benchmark Script
Pulls your ContinElo games directly from Supabase, downloads the
538/Paine Elo dataset, joins them, and reports accuracy/Brier plus
significance tests.

Usage:
    python benchmark_vs_538.py
"""
...
# Empirically-verified team code mapping (see the benchmark report, Section 4)
...
def print_overall(m):
    print("\n" + "=" * 60)
    print(f"OVERALL — {SEASON_START}-{SEASON_END}, identical games")
    print("=" * 60)
...
def main():
    ...
    merged.to_csv("benchmark_merged.csv", index=False)
    season_summary.to_csv("benchmark_by_season.csv")
    print("\nSaved: benchmark_merged.csv, benchmark_by_season.csv")
